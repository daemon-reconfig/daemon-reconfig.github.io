...
export const useDesktopStore = create<DesktopStore>((set, get) => ({
  ...
  openWindow: (type: AppType) => {
    ...
    if (existing) {
      const maxZ = get().windows.reduce<number>((m, w) => Math.max(m, w.zIndex), 0);
      set({
        windows: get().windows.map((w) =>
          w.id === existing.id ? { ...w, isMinimized: false, zIndex: maxZ + 1 } : w
        ),
      });
      return;
    }
    ...
    const winWidth = isMobile ? vw * 0.9 : 460;
    const winHeight = isMobile ? vh * 0.8 : 340;
    const isFullscreen = type === 'resume';
    const x = isFullscreen ? 0 : (vw - winWidth) / 2;
    const y = isFullscreen ? TOP_BAR_HEIGHT : (vh - winHeight) / 2;
    const height = isFullscreen ? vh - TOP_BAR_HEIGHT - TASK_BAR_HEIGHT : winHeight;
  ...
  focusWindow: (id: number) => {
    const maxZ = get().windows.reduce<number>((m, w) => Math.max(m, w.zIndex), 0);
    set({
      windows: get().windows.map((w) => (w.id === id ? { ...w, zIndex: maxZ + 1 } : w)),
    });
  },
  ...
  toggleFullscreen: (id: number) => {
    set({
      windows: get().windows.map((w) => {
        ...
        const isMobile = vw <= 600;
        const restoredWidth = isMobile ? vw * 0.9 : 460;
        const restoredHeight = isMobile ? vh * 0.8 : 340;
        return {
          ...w,
          isFullscreen: false,
          position: {
            x: (vw - restoredWidth) / 2,
            y: (vh - restoredHeight) / 2,
          },
  ...
  moveWindow: (id: number, pos: { x: number; y: number }) => {
    set({
      windows: get().windows.map((w) => (w.id === id ? { ...w, position: pos } : w)),
    });
  },
}));
